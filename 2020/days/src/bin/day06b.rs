...
fn parse(lines: &mut dyn Iterator<Item=Result<String, std::io::Error>>) -> Option<Data> {
	let mut list_of_questions : Vec<HashSet<char>> = vec![];
	loop {
		...
		list_of_questions.push(questions);
	}
	if list_of_questions.is_empty() {
		None
	} else {
		let everyone_questions
			= list_of_questions.iter().fold(
				list_of_questions.first().unwrap().clone(),
				|a, b| a.intersection(&b).cloned().collect()
			);

		Some(Data {
			questions: everyone_questions
		})
	}
}
