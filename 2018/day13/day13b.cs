...
namespace day13
{
	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			List<(int, int)> crashes = new List<(int, int)>();
			...
			while (trains.Count > 1)
			{
				...
				foreach (Train train in trains)
				{
					if (crashed.Contains(train.number)) continue;
					...
					if (locations.ContainsKey((row, col)))
					{
						...
						Console.WriteLine("Trains {0} and {1} crashed at ({2}, {3})", train.number, locations[(row, col)], row, col);
				...
				// Yes.  I realize this is a ridiculous ways to do removes.
				// But silly CSharp has lists that are actually arrays and this isn't going
				// to affect performance at all because # of trains is small versus size
				// of tracks and length of time.
				foreach (int number in crashed)
				{
					int index = 0;
					for (; index < trains.Count; ++index)
					{
						if (number == trains[index].number) break;
					}
					Console.WriteLine("Removing train with number {0} at {1}/{2}", number, index, trains.Count);
			...
			if (trains.Count > 0)
			{
				Console.WriteLine("The only train left is {0} at ({1},{2})",
						trains[0].number, trains[0].column, trains[0].row);
			}
			else Console.WriteLine("There are no trains left.  That seems weird.");
		}
	}
}
